...
$$\lambda_i \langle \psi_j | \psi_i \rangle = \lambda_j^*  \langle \psi_j |   \psi_i \rangle$$ and subtract across on the right to get 
...
Using $E = h\nu$ we can calculate $$\nu = \frac{8.58 \times 10^{-22} ~\text{J}}{6.626 \times 10^{-34} ~\text{J s}} = 1.30 \times 10^{12} ~\text{s}^{-1} (~\text{Hz})$$ Then to find the wavelength of the photon we can use $$\lambda = \frac{c}{\nu} = \frac{2.99 \times 10^8 ~\text{m s}^{-1}}{1.30 \times 10^{12} ~\text{s}^{-1}} = 230 \times 10^{-4} ~\text{m} = 230 ~\text{$\mu$m}$$
...
We begin the derivation with the 3D particle in a box. The Hamiltonian and Energy values for the problem are $$\hat{H} = -\frac{\hbar^2}{2 m} \left( \frac{d}{dx} + \frac{d}{dy} + \frac{d}{dz} \right)$$
